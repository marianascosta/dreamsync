...
fun WaitingForOthersScreen(
    ...
    LaunchedEffect(hikeId) {
        hikeService.getParticipants(hikeId) { fetchedParticipants ->
            ...
            readyCount = participants.count { it.value.hikeStatuses.find { it.hikeId == hikeId }?.status == Status.READY }
    ...
    if (hike == null) {
        ...
        Column(horizontalAlignment = Alignment.CenterHorizontally) {
            ...
            participants.forEach { (id, participant) ->
                val statusForHike = participant.hikeStatuses.find { it.hikeId == hikeId }?.status ?: Status.NOT_READY
